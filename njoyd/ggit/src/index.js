...
program
  ...
  .option('-s, --stash [type]', '使用 stash 暂存区方式合并代码，如果手动终止了脚本，需要使用 $ git stash pop 放出暂存区的代码，以免丢失', config.stash)
...
function pushCurrentBranch(option) {
  ...
  if (isTure(option.stash)) {
    // 输出日志
    BgWarning(`==== 注意：使用 stash 暂存代码，如果手动终止了命令，需使用 $ git stash pop 释放出暂存区代码，以免丢失！`)
    // 输出日志
    BgInfo(`========================================== git stash`)
    // 暂存代码
